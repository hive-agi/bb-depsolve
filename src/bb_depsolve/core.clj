...
(defn audit-cmd
  ...
  (let [{:keys [root skip-dirs depth tree-depth]
    ...
    (println (c :bold "Scanning dependencies for known CVEs (via OSV.dev)..."))
    (println)

    (let [all-deps (collect-all-mvn-deps dep-files cache tree-depth)
          _ (println (str "  Found " (c :cyan (str (count all-deps)))
      ...
      (if (empty? vulnerable)
        (println (c :green "No known vulnerabilities found."))
        (do
          (println (c :bold (c :red (str total-vulns " vulnerabilit"
          ...
          (doseq [[lib {:keys [version vulns]}] (sort-by key vulnerable)]
            (println (str "  " (c :bold (str lib)) " " (c :dim version)))
            (doseq [{:keys [id severity summary fixed-in]} vulns]
              (let [sev-color (case severity
                ...
                (println (str "    " (c sev-color (or severity "?"))
                              " " (c :bold id)
                              "  " (or summary "")
                              (when fix-str (c :green fix-str))))))
            (println))

          (println (c :dim "Source: https://osv.dev"))
          (println)
