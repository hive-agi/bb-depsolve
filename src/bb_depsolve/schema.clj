  ...
  {:bb-depsolve/semver-triple [:tuple :int :int :int]
   ...
   [:and
    [:map
     ...
     [:sha-short {:optional true} :string]
     [:mvn-version {:optional true} :string]]
    [:fn {:error/message "must resolve a git coordinate, a Maven coordinate, or both"}
     (fn [{:keys [tag sha mvn-version]}]
       (or (and tag sha) mvn-version))]]
